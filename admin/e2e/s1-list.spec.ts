...
test.describe('S1 차트 목록', () => {
  test('카드 그리드와 미니 차트, 새 차트 카드가 보인다', async ({ page }) => {
    ...
    // 저장된 차트 기준 batch preview option이 카드별 ECharts 캔버스로 렌더된다. (시드 13개 → 1페이지 8개)
    await expect(page.locator('[data-testid="chart-preview"] canvas')).toHaveCount(8);
  ...
  test('목록 검색으로 차트 카드가 필터된다', async ({ page }) => {
    await page.goto('/');
...
test.describe('S1 필터·정렬·페이지네이션', () => {
  ...
  test('데이터소스 필터로 해당 소스 차트만 남는다', async ({ page }) => {
    ...
    await expect(page).toHaveURL('/charts/analytics-db');
  ...
  test('데이터소스 경로에서 검색해도 선택한 소스 범위를 유지한다', async ({ page }) => {
    await page.goto('/charts/analytics-db');
    await page.getByRole('textbox', { name: '차트 검색' }).fill('방문');
    await expect(page).toHaveURL('/charts/analytics-db?q=%EB%B0%A9%EB%AC%B8');
    await expect(page.locator('#chart-datasource-filter')).toHaveValue('analytics-db');
    await expect(page.getByText('일별 방문자', { exact: true })).toBeVisible();
  });

  test('기존 숫자 ID 필터 주소는 이름 기반 차트 경로로 정규화한다', async ({ page }) => {
    await page.goto('/?datasourceId=1&type=line');
    await expect(page).toHaveURL('/charts/analytics-db?type=line');
    await expect(page.locator('#chart-datasource-filter')).toHaveValue('analytics-db');
  });
  ...
  test('13개 시드에서 페이지네이션이 동작한다', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('[data-testid="chart-card-name"]')).toHaveCount(8);
    await expect(page.getByText('새 차트 만들기')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="chart-card-name"]')).toHaveCount(5);
  ...
  test('마지막 페이지의 유일한 차트를 삭제하면 이전 페이지로 보정된다', async ({ page }) => {
    ...
    await expect(page.getByText('2 / 2')).toBeVisible();
    const lastPageIds = await page.getByRole('button', { name: '편집' }).evaluateAll((buttons) => (
      buttons
        .map((button) => Number(button.closest('a')?.getAttribute('href')?.split('/').at(-1)))
        .filter(Number.isFinite)
    ));
    expect(lastPageIds.length).toBeGreaterThan(1);

    const statuses = await page.evaluate(async (ids) => Promise.all(
      ids.slice(0, -1).map(async (id) => (
        await fetch(`/api/v1/charts/${id}`, { method: 'DELETE' })
      ).status),
    ), lastPageIds);
    expect(statuses).toEqual(lastPageIds.slice(0, -1).map(() => 204));
