...
const columnsOf = (tables: SchemaTable[], table: string | null) => tables.find((t) => t.name === table)?.columns ?? [];
...
export function columnsForBuilder(cfg: BuilderConfig, tables: SchemaTable[]): { value: string; label: string; type: string }[] {
  ...
  return activeTables(cfg).flatMap((t) =>
    columnsOf(tables, t).map((c) => ({ value: `${t}.${c.name}`, label: `${t}.${c.name}`, type: c.type })),
  );
}
...
function joinValidationIssue(cfg: BuilderConfig, tables: SchemaTable[]): string | null {
  ...
  const seen = cfg.table ? [cfg.table] : []; // 체인: 이미 등장한 테이블
  for (const j of joins) {
    if (!j.table) return '조인할 테이블을 선택하세요.';
    if (seen.includes(j.table)) return `같은 테이블을 중복 조인할 수 없습니다: ${j.table}`;
    if (!tables.find((t) => t.name === j.table)) return `존재하지 않는 테이블: ${j.table}`;
    ...
    if (R.table !== j.table) return `조인 대상(ON 오른쪽)은 ${j.table} 의 컬럼이어야 합니다.`;
