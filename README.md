...
문서의 전체 목록과 영역별 권위는 [`문서 안내`](docs/문서안내.md)를 참조한다. 면접 준비는 [`면접 문서 안내`](docs/면접/README.md)에서 시작하고, 상세 사건 원문은 [`개발 문제해결 이력`](docs/개발_문제해결_이력.md), 반복 원인·개선·남은 위험은 [`품질개선 종합분석`](docs/품질개선_종합분석.md)에서 확인한다.
