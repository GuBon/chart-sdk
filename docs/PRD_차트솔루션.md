...
- 데이터 탐색의 각 계층은 같은 차트 목록 UX를 쓴다. `/data/{datasourceName}`은 해당 소스를, `/data/{datasourceName}/{schema}`는 해당 스키마를, `/data/{datasourceName}/{schema}/{relation}`은 해당 관계를 기준 또는 조인으로 참조하는 차트를 기본 탭에서 보여주며 검색·종류·정렬·페이지네이션을 공통 제공한다. 메타데이터 탐색은 각각 `?view=schema`(스키마), `?view=relations`(TABLE·View·Materialized View), `?view=columns`(컬럼) 탭으로 분리한다. 스키마 관계 목록은 대규모 카탈로그에서도 이름 검색을 제공한다. 이름 수정 후에는 새 이름이 정식 URL이 되며 구 이름 주소는 유지하지 않는다.
...
- 옵션 확장 대비: 옵션 패널은 접이식 섹션 + 동작하는 옵션 검색으로 옵션 수십 개 규모까지 같은 레이아웃에서 수용한다(Grafana 패턴). `테마` 명칭은 유지하고 테마·색상·글꼴·차트 고유 외형은 `스타일`, 시리즈 종류·라벨·정렬·범례는 `계열`로 분리한다. 계열 정의가 없는 유형은 빈 계열 탭을 만들지 않는다.
- 지도 편집 탭은 `기본 → 스타일 → 영역 → 계열(있을 때만) → 상호작용 → 데이터` 순서다. 포인트 지도는 계열 설정이 없어 `기본 → 스타일 → 영역 → 상호작용 → 데이터`를 사용한다.
