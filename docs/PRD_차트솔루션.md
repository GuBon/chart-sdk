...
**문서 버전:** v3.2 — 옵션 정보구조·수동 갱신·실백엔드 검증 현행화 (2026-07-27)
**작성일:** 2026-06-10 (v2.6 갱신: 2026-07-16 — TABLE·VIEW·MATERIALIZED VIEW 및 결과 표본 지원)
**기반 문서:** 차트 솔루션 개발 인수인계서 (SDK 설계 명세 v1.0)
**관련 문서:** 화면설계서 v3.7 (Figma 작업용) · API 계약서 v3.4 · 노코드 SQL 생성규칙 v2.8 · 코드구조_가이드 · 다중데이터소스_페더레이션_설계 · 유사서비스_기능비교표 · Flyway V1~V4
...
- MVP 옵션 (섹션별):
  ...
  - 데이터 갱신: 갱신 모드(항상 최신 조회/캐시 사용/수동), 캐시 유효 시간(TTL), [지금 갱신] — 7.7 참조
...
- 임베드 코드: `<div data-chart-id data-auth-token>` + `<script src=".../sdk.js" data-api-base="...">` — chart-id가 차트를, auth-token이 인증을, data-api-base가 SDK 자산 출처와 독립된 Spring API 위치를 가리킨다. S3가 생성한 HTML은 별도 전역 설정이나 초기화 호출 없이 그대로 붙여 넣어 동작해야 한다.
...
```json
{
  ...
  "analysis": {
    "annotations": {
      ...
      "ranges": [{ "id": "range-1", "name": "적정", "seriesIndex": 0, "min": 300, "max": 500, "color": "#FFB000", "showLabel": true, "opacity": 0.16 }],
      "targets": [{ "id": "target-1", "name": "목표", "seriesIndex": 0, "xValue": "가전", "value": 450, "color": "#D81B60", "showLabel": true, "symbol": "pin", "symbolSize": 42 }]
    },
    "boxplotOutliers": { "show": true, "color": "#D81B60" },
    "movingAverage": { "enabled": false, "seriesIndex": 0, "period": 3, "showInLegend": true }
  },
...
전체 기능 범위의 화면과 상태를 모두 정의한다. 우선순위(코어/보조/후속)에 따라 단계적으로 구현한다. 각 화면의 레이아웃·치수·컴포넌트·Figma 프레임 목록을 포함한 상세 설계는 별도 문서 "차트 솔루션 화면설계서 v3.7(Figma 작업용)"를 참조한다. 본 장은 그 요약이다.

### 11.0 UI 구조 결정 사항

- 내비게이션: 사이드바 없이 얇은 상단 바(56px) 하나로 통일한다. 이동 목적지가 적고(목록·설정), S2 편집 화면의 가로 공간 확보가 우선이다. 화면이 늘어나면 접이식 사이드바로 전환한다.
- S2 편집은 정의와 차트를 분리하지 않고 3분할 한 화면(테이블·컬럼 패널 / 노코드 구성+결과 탭 / 차트+옵션 — SQL 탭(2차)에선 스키마 탐색기/에디터+결과)을 유지한다. `노코드/SQL` 정의 모드 탭은 전역 줄이 아니라 중앙 구성 패널 내부에 둔다. 우측은 미리보기 위·옵션 아래 순서를 유지하며 경계 드래그와 옵션 접기를 지원한다. 데이터 패널과 노코드 구성·결과 패널은 각각 독립적으로 접고, 접힌 패널의 원래 위치에 남는 좁은 좌측 복원 레일에서 다시 펼친다. 중앙을 접으면 차트+옵션이 남은 폭을 채우고 두 패널을 모두 접으면 두 복원 레일을 제외한 편집 폭을 사용한다. 최종 검수는 전체 viewport 포커스 미리보기로 수행한다. 구성 수정과 차트 확인의 왕복을 없애기 위함이며, Grafana·Metabase·Redash가 같은 구조다. 쿼리를 독립 엔터티로 분리(재사용 1:N)하는 것은 수요 확인 시 후속으로 검토한다.
- 미리보기 패널에는 고정 최대 폭을 두지 않고 사용자가 현재 viewport 안에서 원하는 만큼 넓힌다. 드래그로 데이터 패널 또는 노코드 구성·결과 패널의 잔여 폭을 48px 이하까지 줄이면 해당 패널을 자동으로 접어, 별도 접기 버튼을 다시 찾지 않아도 같은 드래그 흐름으로 미리보기 공간을 확보한다.
- 옵션 확장 대비: 옵션 패널은 접이식 섹션 + 동작하는 옵션 검색으로 옵션 수십 개 규모까지 같은 레이아웃에서 수용한다(Grafana 패턴). `테마` 명칭은 유지하고 테마·시리즈 색상·글꼴·차트 고유 외형은 `스타일`, 시리즈 종류·라벨·정렬·범례는 `계열`로 분리한다. 지도 색상도 공통 시리즈 색상 편집을 사용하며, 포인트 모양은 `스타일 → 점`, 배경 행정구역의 표시·채우기·윤곽선은 `영역 → 행정 경계`에서 제공한다. 계열 정의가 없는 유형은 빈 계열 탭을 만들지 않는다.
- 모든 차트의 편집 탭은 `기본 → 계열 → 축/영역 → 스타일 → 상호작용 → 데이터`의 같은 상대 순서를 사용한다. 지도와 포인트 지도는 축 대신 영역을 사용해 `기본 → 계열 → 영역 → 스타일 → 상호작용 → 데이터`로 표시한다.
...
| S5D | 데이터 카탈로그 | 관리 콘솔 | 코어 | 데이터소스→스키마→TABLE/View/Materialized View→관련 차트 탐색 |
| S6 | 로그인 (임시 자체) | 관리 콘솔 | 골격만 | 추후 SSO 교체, 개발 중 우회 가능 |
| S7 | 토큰 관리 | 관리 콘솔 | 코어 | 사용자별 임베드 토큰 발급/회수 |

### 11.2 화면별 상태 요약

상세는 화면설계서 v3.7 참조. 핵심 상태만 요약한다.

- S1: 기본 / 빈 상태 / 삭제 확인(임베드 영향 경고). 카드에 차트 ID(#id)·설명 1줄 표시. 페이지 헤더에 검색(이름·설명 부분일치). 카드 호버 액션에 복제(2차)
- S2 추가 상태: 미저장 이탈 확인 모달. 데이터 탭의 [지금 갱신]은 신규 차트·미저장 변경 상태에서 비활성 안내, 실행 중, 성공(마지막 계산 시각), refresh 실패, refresh 성공 후 preview 재조회 실패 상태를 가진다.
- S2(노코드, MVP): 신규(데이터소스 미선택·셀렉트 펼침 포함) / 구성 중(테이블 선택 시 [원본 데이터] 탭 자동 표시 — 최대 1,000행 스크롤) / 로딩 / 성공([실행 결과]·[원본 데이터] 탭 전환 — 집계와 세부 데이터 모두 확인) / 에러(생성 전 검증 400, 친화적 문구 — DB 에러 미노출) / 저장 / 소스변경확인 모달(구성 초기화 경고). 실행 실패 시 차트는 이전 상태 유지. 개별 색 지정은 실행 성공 후 활성(동적 UI). 미리보기는 논리 크기 선택, 화면/너비 맞춤, 100%·zoom, 데이터·노코드·옵션 패널별 접기, 전체 화면 포커스 상태를 지원한다
...
- 후속: SSO 교체, 쿼리 파라미터, 대시보드, 알림, 캐시 스케줄·실행 이력.
