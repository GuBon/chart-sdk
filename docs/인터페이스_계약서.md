...
목록용 `GET /charts/previews?ids=...`는 최대 60개 카드 응답이므로 `option`과 표시 메타데이터만 반환하고 `columns/rows/elapsedMs`는 포함하지 않는다. `manual`은 마지막 스냅샷을 배치로 읽고, 단일 소스 `live`는 `serve()`로 재계산한다.
