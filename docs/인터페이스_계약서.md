...
4. 키의 `(user_id, chart_id)` 바인딩으로 차트를 조회한다. 소유자 범위는 발급 시점에 검증되었고, 서빙에서도 `owner_id` 일치(로그인 전 레거시 NULL 호환 포함)를 재확인한다 → 없으면 404
...
원문·해시를 저장하지 않고 서버 비밀키(`chartsdk.embed.key-secret`, 환경변수 `CHARTSDK_EMBED_KEY_SECRET`,
미설정 시 jwt-secret)로 행 id에서 재파생한다. 원문은 발급 성공 응답에서만 한 번 반환하고 목록에는 포함하지 않는다.

### 4.0.1 발급
```
POST /api/v1/charts/{chartId}/embed-keys
```
요청: { "userId": 7, "expiresInDays": 365 }  (expiresInDays 선택, 1~3650)
...
- 발급 시점에 사용자 활성 여부(404 USER_NOT_FOUND)와 차트 소유자 범위(타인 소유 차트는 존재를 숨기고
  404 CHART_NOT_FOUND)를 검증한다. 로그인 도입 전 `owner_id IS NULL` 레거시 차트는 발급을 허용한다.
...
## 4-레거시. 사용자 임베드 토큰 (S7 토큰 관리)

> **임베드 인증에서 제외됨**: 사용자 토큰(JWT)은 더 이상 `GET /api/v1/charts/data`를 인증하지 못한다.
> S7 발급·회수 API와 화면은 유지되지만, 임베드 스니펫은 위 4장의 임베드 키만 사용한다.
> 이 토큰의 존치 여부는 관리 콘솔 로그인 트랙에서 재결정한다.
...
- 키 형식은 `cek1_<id>_<sig>`, `sig = base64url(HMAC-SHA256(key-secret, "chartsdk:embed-key:v1:" + id))`다. 비밀키 설정은 `chartsdk.embed.key-secret`, 운영 환경 변수는 `CHARTSDK_EMBED_KEY_SECRET`(미설정 시 jwt-secret 승계)이다. 개발 기본 비밀키는 운영에서 사용할 수 없다.
...
- ①~③은 `EmbedKeyInterceptor` 한 곳에서 처리하고 검증된 `EmbedKeyPrincipal`을 요청에 전달한다. 차트 조회는 키 바인딩으로만 수행하며 클라이언트는 chartId를 보낼 수 없다. 로그인 도입 전 호환용 `owner_id IS NULL` 경로는 발급·서빙 양쪽에서 임시 유지하며 소유권 마이그레이션이 끝나면 제거한다.
- 비밀키와 키 원문은 로그에 남기지 않는다. 모든 오류는 공통 오류 계약으로 정규화하며, 다른 사용자의 차트는 존재 여부를 숨기기 위해 `404 CHART_NOT_FOUND`로 처리한다.
- 임베드 API의 CORS는 승인한 사내 호스트로 한정한다. 지도 정적 자산은 키 없이 제공하되 별도의 정적 자산 CORS·캐시 정책을 따른다.
