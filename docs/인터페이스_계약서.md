...
- 일반 콘솔 API는 PostgreSQL 세션의 로그인 `userId`로 자동 스코프한다. 관리자 자신의 차트·데이터소스도 같은 규칙을 적용한다.
...
- 비밀번호는 Unicode 코드 포인트 기준 최소 15자만 검사하고 복잡도 규칙은 적용하지 않는다. Argon2id 해시만 DB에 저장한다.
