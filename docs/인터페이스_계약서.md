...
- 일반 콘솔 API는 PostgreSQL 세션의 로그인 `userId`로 자동 스코프한다. 관리자 자신의 차트·데이터소스도 같은 규칙을 적용한다. 예외는 읽기 목록 두 개뿐이다 — `GET /charts`(각 항목에 `ownerId` 포함, `admin`에게는 검색어 `q`가 소유자 아이디·표시 이름·숫자 id에도 매칭)와 `GET /charts/previews`는 `admin`에게 모든 사용자 범위로 열리며, 타인 차트의 카드 미리보기는 저장 스냅샷만 쓴다(live 재계산 없음). 상세·저장·삭제·복제·갱신·임베드 키는 계속 소유자만 통과한다.
