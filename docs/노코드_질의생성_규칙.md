...
**문서 버전:** v3.0 — 문서 책임 분리·표본추출 계약 연결 (2026-08-06)<br>
**관련 문서:** [제품요구사항](제품요구사항.md) · [인터페이스 계약서](인터페이스_계약서.md) · [화면설계서](화면설계서.md) · [표본추출 계약](표본추출_계약.md)
**대상 DB:** PostgreSQL 고정
...
|---|---|---|---|---|
| 사용자 입력으로 안전한 SQL 생성 | 구조 enum whitelist·catalog identifier whitelist·값 binding | 문자열 sanitize, raw SQL만 제공, ORM Criteria | SQL 구조와 값을 분리하고 외부 DB의 동적 schema도 검증 가능 | 지원 연산자·집계 범위 제한. SQL snapshot·실DB 테스트 |
| 동명 relation과 다중 source 식별 | 구조화 `TableRef`+차트 내 유일 handle | `datasource.schema.table` 문자열, 항상 SQL alias | 점 파싱 모호성을 없애고 충돌 없는 기존 SQL은 바꾸지 않음 | 자기조인은 SQL alias가 필요해 후속 |
| 집계와 원본 상세의 부하 분리 | `mode:aggregate` 명시 실행, `mode:rows` 지연 1,000행 | 실행 때 둘 다 조회, 하나의 범용 endpoint | 사용자가 보지 않는 raw query를 피하면서 같은 WHERE binding을 재사용 | raw download/page 기능은 후속 |
| 고객 DB 무손상 | read-only 실행, VIEW/MV 생성 안 함 | 최적화 VIEW·임시 테이블 생성 | Type B 운영 DB에 객체·권한 변경을 요구하지 않음 | 복잡한 연합 쿼리는 DuckDB 계산 비용 발생 |

보안 사건별 원인과 조치는 [개발 문제해결 이력](개발_문제해결_이력.md)을 참조한다.
...
| sample | — | `{ mode:"auto"\|"manual", size?, method?:"auto"\|"system", rate?, seed }`. 저장 스키마와 값 범위는 [표본추출 계약](표본추출_계약.md)을 따르며 집계와 `agg:"none"` 원본 행 모드 모두에서 사용할 수 있다. |
...
`builderConfig.sample`은 자동 또는 1,000~50,000행 직접 지정 방식으로 차트 입력 행을 선택한다. SQL 생성기는 저장된 설정을 검증해 `SamplingPlanner`에 전달하며, 관계 유형과 실행 결과에 따라 INDEX_RANDOM·RESULT_RANDOM·RESERVOIR_RANDOM·SYSTEM·FULL_SCAN 중 하나가 사용된다.

표본 방식, 통계적 의미, sampling v9 응답, 캐시 호환성과 사용자 표시 문구의 정본은 [표본추출 계약](표본추출_계약.md)이다. 이 문서는 다음 SQL 생성 경계만 소유한다.

- 관계 미리보기와 `mode:"rows"`에는 sample을 적용하지 않는다.
- `agg:"none"`과 sample은 함께 허용하며 원본 행 표본으로 실행한다.
- 단일 물리 관계는 planner가 선택한 PostgreSQL 경로를 사용한다.
- VIEW와 JOIN+WHERE 결과는 결과 행을 모집단으로 삼는 RESULT_RANDOM CTE를 생성한다.
- 다중 데이터소스 JOIN은 같은 논리 CTE를 DuckDB table renderer로 출력한다.
- 공간 Point/Polygon 컬럼 모드는 별도 geometry 표본 계약 전까지 sample과 함께 쓰지 않는다.
- 잘못된 `mode`, `method`, `rate`, `size`, `seed`는 SQL 생성 전에 `400 INVALID_REQUEST`로 거부한다.
## 4. WHERE 연산자(op) 목록
